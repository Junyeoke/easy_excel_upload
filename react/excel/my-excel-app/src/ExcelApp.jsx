...
<div style={{ marginTop: '8px', fontSize: '0.75rem', color: '#64748b', 
  ...
  // ─── 렌더: 사용자 STEP 1 (파일선택 + 미리보기 + 업로드 통합) ─
  const renderUserStep1 = () => (
    <div className="wizard-panel">
      <div className="wizard-panel-title">📂 파일 선택 및 업로드</div>
      <div className="wizard-panel-desc">
        {uploading ? '데이터를 서버에 전송 중입니다. 완료될 때까지 창을 닫지 마세요.'
          : uploadResult ? '업로드가 완료되었습니다.'
          : '업로드할 엑셀 파일을 선택하고 데이터를 확인 후 업로드하세요.'}
      </div>
      ...
      {uploadResult ? (
        <div style={{ marginTop: '24px' }}>
          {uploadResult.status === 'ok' && (
            <div style={{ textAlign: 'center', padding: '36px 20px', background: '#f0fdf4', border: '1px solid #a7f3d0', borderRadius: '16px' }}>
              <div style={{ fontSize: '3.2rem', marginBottom: '12px' }}>✅</div>
              <div style={{ fontSize: '1.2rem', fontWeight: 700, color: '#065f46', marginBottom: '6px' }}>업로드 완료!</div>
              <div style={{ color: '#059669', fontSize: '0.9rem' }}>{uploadResult.msg}</div>
            </div>
          )}
          {uploadResult.status === 'partial' && (
            <div style={{ padding: '24px 20px', background: '#fffbeb', border: '1px solid #fde68a', borderRadius: '16px', textAlign: 'center' }}>
              <div style={{ fontSize: '2.8rem', marginBottom: '10px' }}>⚠️</div>
              <div style={{ fontSize: '1.1rem', fontWeight: 700, color: '#92400e', marginBottom: '12px' }}>일부 실패</div>
              <div style={{ display: 'flex', gap: '10px', justifyContent: 'center', flexWrap: 'wrap', marginBottom: '16px' }}>
                <span style={{ background: '#d1fae5', color: '#065f46', padding: '4px 14px', borderRadius: '20px', fontWeight: 700, fontSize: '0.9rem' }}>✅ {uploadResult.success_cnt?.toLocaleString()}건 성공</span>
                <span style={{ background: '#fee2e2', color: '#991b1b', padding: '4px 14px', borderRadius: '20px', fontWeight: 700, fontSize: '0.9rem' }}>❌ {uploadResult.fail_cnt?.toLocaleString()}건 실패</span>
              </div>
              {uploadResult.error_file && <button onClick={() => downloadErrorReport(uploadResult.error_file)} style={{ padding: '10px 20px', borderRadius: '8px', border: 'none', background: '#ef4444', color: 'white', fontWeight: 700, cursor: 'pointer', fontSize: '0.88rem' }}>🚨 오류 리포트 다운로드</button>}

              {/* ── 오류 행 목록 표 ── */}
              {uploadResult.failed_row_msgs && Object.keys(uploadResult.failed_row_msgs).filter(k => k !== '__unknown__').length > 0 && (
                <div style={{ marginTop: '18px', textAlign: 'left' }}>
                  <div style={{ fontWeight: 700, color: '#92400e', fontSize: '0.9rem', marginBottom: '8px' }}>📋 오류 발생 행 목록</div>
                  <div style={{ maxHeight: '200px', overflowY: 'auto', border: '1px solid #fde68a', borderRadius: '8px', background: 'white' }}>
                    <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: '0.82rem' }}>
                      <thead>
                        <tr style={{ background: '#fef3c7' }}>
                          <th style={{ padding: '7px 12px', textAlign: 'center', borderBottom: '1px solid #fde68a', whiteSpace: 'nowrap', color: '#92400e', fontWeight: 700, width: '80px' }}>행 번호</th>
                          <th style={{ padding: '7px 12px', textAlign: 'left', borderBottom: '1px solid #fde68a', color: '#92400e', fontWeight: 700 }}>오류 내용</th>
                        </tr>
                      </thead>
                      <tbody>
                        {Object.entries(uploadResult.failed_row_msgs)
                          ...
                          .map(([rowNum, errMsg]) => (
                            <tr key={rowNum} style={{ borderBottom: '1px solid #fef9c3' }}>
                              <td style={{ padding: '6px 12px', textAlign: 'center', color: '#b91c1c', fontWeight: 700 }}>{rowNum}행</td>
                              <td style={{ padding: '6px 12px', color: '#7c2d12', wordBreak: 'break-all' }}>
                                {translateError(errMsg).friendlyMsg}
                                {translateError(errMsg).solution && (
                                  <div style={{ marginTop: '3px', fontSize: '0.78rem', color: '#166534' }}>💡 {translateError(errMsg).solution}</div>
                                )}
                              </td>
                            </tr>
                          ))}
                  ...
                  <button onClick={() => {
                    if (uploadResult.failed_row_msgs && Object.keys(uploadResult.failed_row_msgs).length > 0) {
                      ...
                      if (validKeys.length > 0) {
                        ...
                        const pg = Math.ceil(firstFail / previewPageSize);
                        if (pg !== previewPage && pg >= 1) { setPreviewPage(pg); fetchPreviewPage(file, pg, headerRow, null); }
                      }
                    }
                    setUploadResult(null);
                  }} style={{ padding: '10px 20px', borderRadius: '8px', border: 'none', background: '#f59e0b', color: 'white', fontWeight: 700, cursor: 'pointer', fontSize: '0.88rem' }}>
                    ✏ 미리보기에서 실패 행 직접 수정하기
                  </button>
                )}
              </div>
          ...
          {uploadResult.status === 'err' && (
            <div style={{ padding: '20px', background: '#fff1f2', border: '1px solid #fecdd3', borderRadius: '16px' }}>
              <div style={{ fontWeight: 700, color: '#e11d48', marginBottom: '8px' }}>❌ 오류 발생</div>
              <div style={{ color: '#9f1239', fontSize: '0.87rem', marginBottom: '10px' }} dangerouslySetInnerHTML={{ __html: uploadResult.friendlyMsg || uploadResult.msg }} />
              {/* 디버그용 — 원인 파악 후 제거 */}
<div style={{ marginTop: '8px', fontSize: '0.75rem', color: '#64748b', 
              background: '#f1f5f9', padding: '8px', borderRadius: '6px',
              fontFamily: 'monospace', wordBreak: 'break-all' }}>
  🔍 원문: {uploadResult.msg}
              ...
              {uploadResult.solution && <div style={{ color: '#166534', background: '#f0fdf4', padding: '10px 14px', borderRadius: '8px', fontSize: '0.84rem' }}>💡 {uploadResult.solution}</div>}
              <button onClick={() => setUploadResult(null)}
                style={{ marginTop: '12px', padding: '9px 18px', borderRadius: '8px', border: '1px solid #fca5a5', background: 'white', color: '#e11d48', fontWeight: 700, cursor: 'pointer', fontSize: '0.84rem', width: '100%' }}>
                ✏ 미리보기에서 직접 수정하기
              </button>
            </div>
          )}
          <div style={{ textAlign: 'center', marginTop: '20px' }}>
            <button onClick={() => { setUploadResult(null); setUploadLogs([]); setProgress({current:0,total:0,percent:0}); setFailedRows({}); }}
              style={{ padding: '11px 24px', borderRadius: '10px', border: '1px solid #e2e8f0', background: 'white', color: '#475569', fontWeight: 600, cursor: 'pointer', fontSize: '0.9rem' }}>
              ← 다른 파일 업로드
            </button>
          </div>
        ...
        <div style={{ marginTop: '24px' }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '8px' }}>
            <span style={{ fontSize: '0.88rem', fontWeight: 600, color: '#1e293b' }}>진행 상황</span>
            <span style={{ fontSize: '0.88rem', fontWeight: 700, color: '#6366f1' }}>{progress.percent}%</span>
          </div>
          <div style={{ height: '12px', background: '#e2e8f0', borderRadius: '100px', overflow: 'hidden' }}>
            <div style={{ height: '100%', borderRadius: '100px', background: 'linear-gradient(90deg,#6366f1,#8b5cf6)', width: `${progress.percent}%`, transition: 'width 0.4s ease' }} />
          </div>
          <div style={{ marginTop: '5px', fontSize: '0.76rem', color: '#64748b', textAlign: 'right' }}>{progress.current.toLocaleString()} / {progress.total.toLocaleString()} 건</div>
          <div style={{ marginTop: '14px', background: '#0f172a', borderRadius: '10px', padding: '12px 14px', maxHeight: '200px', overflowY: 'auto', fontFamily: 'monospace', fontSize: '0.75rem' }}>
            {uploadLogs.map((log, i) => (
              <div key={i} style={{ marginBottom: '2px', color: log.includes('❌') ? '#f87171' : log.includes('🎉') ? '#fbbf24' : '#4ade80', paddingBottom: '2px', borderBottom: '1px solid rgba(255,255,255,0.05)' }}>{log}</div>
            ))}
            <div ref={logEndRef} />
        ...
        <>
          <div style={{ marginTop: '24px' }}>
            {file ? (
              <div style={{ background: '#f0fdf4', border: '1.5px solid #6ee7a0', borderRadius: '14px', padding: '14px 18px', display: 'flex', alignItems: 'center', gap: '14px' }}>
                <div style={{ width: '42px', height: '42px', borderRadius: '10px', background: 'white', border: '1px solid #a7f3d0', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '1.3rem', flexShrink: 0 }}>📊</div>
                <div style={{ flex: 1, minWidth: 0 }}>
                  <div style={{ fontWeight: 700, color: '#065f46', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{file.name}</div>
                  <div style={{ display: 'flex', gap: '6px', marginTop: '4px', flexWrap: 'wrap' }}>
                    {[formatFileSize(file.size), `헤더 ${excelHeaders.length}개`, `총 ${totalRows.toLocaleString()}건`].map(t => (
                      <span key={t} style={{ fontSize: '0.7rem', background: 'white', border: '1px solid #a7f3d0', color: '#059669', padding: '2px 8px', borderRadius: '20px', fontWeight: 600 }}>{t}</span>
                    ))}
                  </div>
                </div>
                <button onClick={() => { setFile(null); setExcelHeaders([]); setPreviewData([]); setTotalRows(0); setEditedCells({}); setFailedRows({}); const el=document.getElementById('fileInput'); if(el) el.value=''; }}
                  style={{ height: '30px', padding: '0 12px', borderRadius: '8px', border: '1px solid #fca5a5', background: 'white', color: '#ef4444', fontSize: '0.78rem', fontWeight: 700, cursor: 'pointer' }}>✕ 제거</button>
              </div>
            ) : (
              <label htmlFor="fileInput"
                ...
                onDragLeave={e => { e.preventDefault(); setIsDragging(false); }}
                onDrop={e => { e.preventDefault(); setIsDragging(false); const f=e.dataTransfer.files[0]; if(f&&(f.name.endsWith('.xlsx')||f.name.endsWith('.xls'))) processSelectedFile(f); else if(f) toast.error('❌ .xls/.xlsx만 가능합니다.'); }}
                style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', minHeight: '150px', borderRadius: '14px', cursor: 'pointer', border: isDragging ? '2px solid #6366f1' : '2px dashed #cbd5e1', background: isDragging ? '#eef2ff' : '#f9fafb', transition: 'all 0.18s', gap: '10px', padding: '24px' }}>
                <input type="file" id="fileInput" accept=".xls,.xlsx" style={{ display: 'none' }} onChange={e => { const f=e.target.files[0]; if(f) processSelectedFile(f); else { setExcelHeaders([]); setPreviewData([]); setTotalRows(0); }}} />
                <div style={{ width: '46px', height: '46px', borderRadius: '12px', background: isDragging ? '#6366f1' : 'white', border: isDragging ? 'none' : '1.5px solid #e5e8eb', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '1.4rem' }}>{isDragging ? '📂' : '📤'}</div>
                <div style={{ textAlign: 'center' }}>
                  <div style={{ fontWeight: 700, color: isDragging ? '#6366f1' : '#333d4b' }}>{isDragging ? '여기에 놓아주세요!' : '파일을 드래그하거나 클릭하여 선택'}</div>
                  <div style={{ marginTop: '5px', display: 'flex', gap: '5px', justifyContent: 'center' }}>
                    {['.xls', '.xlsx'].map(ext => <span key={ext} style={{ background: '#eef2ff', color: '#6366f1', fontSize: '0.7rem', fontWeight: 700, padding: '2px 8px', borderRadius: '5px' }}>{ext}</span>)}
                    <span style={{ color: '#b0b8c1', fontSize: '0.73rem' }}>· DRM 해제 필수</span>
                  </div>
                </div>
              </label>
            )}
          ...
          {/* 미리보기 */}
          {previewData.length > 0 && (
            <div style={{ marginTop: '18px', border: '1px solid #e5e8eb', borderRadius: '12px', overflow: 'hidden' }}>
              {/* 실패 행 배너 */}
              {Object.keys(failedRows).length > 0 && (
                <div style={{ background: '#fff1f2', borderBottom: '1px solid #fecdd3', padding: '10px 14px', display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexWrap: 'wrap', gap: '8px' }}>
                  <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
                    <span style={{ fontSize: '1rem' }}>🚨</span>
                    <span style={{ fontWeight: 700, color: '#e11d48', fontSize: '0.85rem' }}>
                      {failedRows['__unknown__'] ? `${uploadResult?.fail_cnt ?? '일부'}건 실패` : `${Object.keys(failedRows).length}행 실패`}
                    </span>
                    <span style={{ color: '#9f1239', fontSize: '0.78rem' }}>
                      {failedRows['__unknown__'] ? '오류 리포트를 다운로드하거나 수정 후 재업로드하세요.' : '빨간 행을 수정한 후 재업로드하세요.'}
                    </span>
                  </div>
                  <div style={{ display: 'flex', gap: '6px', alignItems: 'center' }}>
                    <button onClick={() => {
              ...
              <div style={{ background: '#f9fafb', padding: '10px 14px', borderBottom: '1px solid #e5e8eb', display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: '8px' }}>
                <span style={{ fontWeight: 700, fontSize: '0.85rem', color: '#191f28', display: 'flex', alignItems: 'center', gap: '6px' }}>
                  📋 데이터 미리보기
                  {Object.keys(failedRows).length > 0 && <span style={{ color: '#e11d48', fontSize: '0.76rem' }}>· 🚨 {Object.keys(failedRows).length}행 실패</span>}
                  {Object.keys(editedCells).length > 0 && <span style={{ color: '#e67e22', fontSize: '0.76rem' }}>· ✏ {Object.keys(editedCells).length}행 수정됨</span>}
                </span>
                <div style={{ display: 'flex', gap: '8px', alignItems: 'center' }}>
                  {Object.keys(editedCells).length > 0 && <button onClick={() => setEditedCells({})} style={{ fontSize: '0.73rem', padding: '3px 9px', borderRadius: '6px', border: '1px solid #fca5a5', background: 'white', color: '#ef4444', cursor: 'pointer' }}>↩ 수정 초기화</button>}
                  <span style={{ fontSize: '0.76rem', fontWeight: 700, color: '#3182f6', background: '#ebf3ff', padding: '3px 9px', borderRadius: '20px' }}>총 {totalRows.toLocaleString()}건</span>
                </div>
              </div>
              <div style={{ overflowX: 'auto', maxHeight: '300px', position: 'relative' }}>
                {previewLoading && <div style={{ position: 'absolute', inset: 0, background: 'rgba(255,255,255,0.7)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 2, fontWeight: 600, color: '#3182f6' }}>로딩 중...</div>}
                  ...
                  <tbody>
                    {previewData.map((row, rIdx) => {
                      ...
                      return (
                        <React.Fragment key={rIdx}>
                          <tr style={{ background: isFailed ? '#fff1f2' : isEdited ? '#fffbeb' : undefined, boxShadow: isFailed ? 'inset 3px 0 0 #ef4444' : undefined }}>
                            <td style={{ textAlign: 'center', color: isFailed ? '#e11d48' : '#b0b8c1', background: isFailed ? '#fee2e2' : isEdited ? '#fef9c3' : '#f9fafb', fontWeight: 700, borderRight: '1px solid #e5e8eb', fontSize: '0.76rem', minWidth: '46px', verticalAlign: 'middle' }}>
                              {rowNum}
                              {isFailed && <span style={{ display: 'block', fontSize: '0.65rem', color: '#e11d48' }}>❌</span>}
                              {!isFailed && isEdited && <span style={{ color: '#e67e22' }}>✏</span>}
                            </td>
                            {excelHeaders.map((_, cIdx) => {
                              const orig = Array.isArray(row) ? (row[cIdx]||'') : (row[String(cIdx)]||'');
                              ...
                              return (
                                <td key={cIdx} style={{ padding: 0, position: 'relative' }}>
                                  ...
                                  <input value={disp} onChange={e => { const nv=e.target.value; setEditedCells(prev => { const pr=prev[rowNum]||{}; if(nv===orig){const {[cIdx]:_,...rest}=pr;if(!Object.keys(rest).length){const {[rowNum]:__,...rr}=prev;return rr;}return {...prev,[rowNum]:rest};}return {...prev,[rowNum]:{...pr,[cIdx]:nv}};}); }}
                                    ...
                                    onFocus={e => { e.target.style.background = '#f0f7ff'; }} onBlur={e => { e.target.style.background = isCE ? '#fef3c7' : isErrCol ? '#fee2e2' : isFailed ? '#fff8f8' : 'transparent'; }} />
                                </td>
                              );
                            })}
                          </tr>
              ...
              {previewTotalPages > 1 && (
                <div style={{ padding: '8px 14px', borderTop: '1px solid #e5e8eb', display: 'flex', alignItems: 'center', justifyContent: 'space-between', background: '#f9fafb', flexWrap: 'wrap', gap: '6px' }}>
          ...
          {/* 업로드 버튼 - 파일이 선택되면 바로 표시 */}
          {file && (() => {
            const hasFailed = Object.keys(failedRows).length > 0;
            return (
              <div style={{ marginTop: '24px', padding: '20px 24px', background: hasFailed ? 'linear-gradient(135deg,#fff1f2,#fef2f2)' : 'linear-gradient(135deg,#eef2ff,#f5f3ff)', borderRadius: '14px', border: hasFailed ? '1px solid #fecdd3' : '1px solid #e0e7ff', display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: '16px', flexWrap: 'wrap' }}>
                <div>
                ...
                <div style={{ display: 'flex', gap: '10px', alignItems: 'center', flexWrap: 'wrap' }}>
                  ...
                  <button onClick={handleUpload} disabled={previewLoading}
                    style={{ padding: '13px 32px', borderRadius: '12px', border: 'none', background: previewLoading ? '#e2e8f0' : hasFailed ? 'linear-gradient(135deg,#ef4444,#dc2626)' : 'linear-gradient(135deg,#6366f1,#8b5cf6)', color: previewLoading ? '#94a3b8' : 'white', fontWeight: 800, cursor: previewLoading ? 'not-allowed' : 'pointer', fontSize: '0.95rem', display: 'inline-flex', alignItems: 'center', gap: '8px', boxShadow: previewLoading ? 'none' : hasFailed ? '0 4px 14px rgba(239,68,68,0.35)' : '0 4px 14px rgba(99,102,241,0.35)', whiteSpace: 'nowrap' }}>
                    {hasFailed ? '🔄 수정 후 재업로드' : '🚀 업로드 실행'}
                  </button>
                </div>
              </div>
