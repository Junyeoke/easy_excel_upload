...
function App() {
  ...
  const loadTables = () => post("/api/easy-sync/ui", { mode: "get_tables" }).then(d => setTableList(Array.isArray(d) ? d : []));
  const loadJobList = () => post("/api/easy-sync/engine", { mode: "get_job_list" }).then(d => setJobs(Array.isArray(d) ? d : []));

  const loadCols = async (tbl) => {
    if (!tbl) return;
    const decodedTbl = decodeEntity(tbl);
    const d = await post("/api/easy-sync/ui", { mode: "get_columns", table_name: decodedTbl });
  ...
  const handleLoadJob = async (id) => {
    const data = await post("/api/easy-sync/engine", { mode: "get_job_detail", sync_id: id });
  ...
  const handleSave = async () => {
    ...
    const res = await post("/api/easy-sync/engine", payload);
    setIsSaving(false);
  ...
  const handleRun = async () => {
    ...
    const res = await post("/api/easy-sync/engine", { mode: "run_job", sync_id: form.sync_id });
    setIsRunning(false);
  ...
  const handleDelete = async () => {
    ...
    if (!ok) return;
    await post("/api/easy-sync/engine", { mode: "delete", sync_id: form.sync_id });
            ...
            >
              ...
              <div className="form-row" style={{ marginBottom: 14 }}>
                <div className="form-col">
                  ...
                  <div style={{ display: 'flex', gap: 12, marginBottom: 6, fontSize: '0.8rem' }}>
                    <label style={{ fontWeight: 500, color: '#475569', cursor: 'pointer' }}>
                      <input type="radio" checked={srcType === 'TABLE'} onChange={() => setSrcType('TABLE')} style={{ marginRight: 4 }} />
                      테이블 선택
                    </label>
                    <label style={{ fontWeight: 500, color: '#475569', cursor: 'pointer' }}>
                      <input type="radio" checked={srcType === 'SQL'} onChange={() => setSrcType('SQL')} style={{ marginRight: 4 }} />
                      커스텀 SQL
                    </label>
                  </div>
                  {srcType === 'TABLE'
                ...
                <div style={{ display: 'flex', alignItems: 'center', color: '#818cf8', fontSize: '1.2rem', paddingTop: 24, flexShrink: 0 }}>→</div>
                <div className="form-col">
                  <label style={{ color: '#e11d48' }}>💾 대상 테이블 (Target) <span style={{ color: '#ef4444' }}>*</span></label>
                  <div style={{ height: srcType === 'SQL' ? 90 : 'auto' }} />
